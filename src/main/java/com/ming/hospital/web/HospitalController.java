...
public class HospitalController {
    ...
    public Result selectById( @PathVariable @RequestParam(required = false)Integer id ) {
        Hospital hospital = hospitalService.selectById(id);
        return Result.ok(hospital,"添加数据成功");
    }
    ...
    @ResponseBody
    @ApiOperation(value = "添加数据")
    public Result selectByPage( @PathVariable @RequestParam(required = false)String hname,
                               @RequestParam(required = false)Integer pageNo,
                               @RequestParam(required = false)Integer pageSize) {
        pageNo = pageNo == null ? 1 : pageNo;
        pageSize = pageSize == null ? 3 : pageSize;
        PageHelper.startPage(pageNo, pageSize);
        List<Hospital> hospitals = hospitalService.selectByPage(hname);
